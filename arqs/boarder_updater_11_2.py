...
def process_frame(frame, detector, transformer, object_line_detector, previous_board, board_accumulator, already_printed, hand_presence):
    # Processa a jogada somente se a mão não estiver presente
    if hand_presence:
        print("Mão detectada, aguarde para registrar a jogada.")
        return frame, previous_board, False  # Retorna False para indicar que nenhuma jogada foi processada
        ...
        if labeled_points is not None:
            ...
            if len(board_accumulator) >= 50:
                ...
                if previous_board is not None:
                    detect_move(previous_board, average_board, already_printed)
                    print(np.array(average_board)) 
                    previous_board = average_board
                else:
                    previous_board = average_board
                return frame, previous_board, True 
    return frame, previous_board, False

def board_frame(frame, detector, transformer, object_line_detector, previous_board, already_printed):
    corners, ids = detector.find_aruco_markers(frame)
    if ids is not None and corners:
        closest_points = detector.find_closest_point_to_center(frame, corners)
        labeled_points = detector.associate_points_with_ids(closest_points, ids, frame)
        if labeled_points is not None:
            ...
            cv2.waitKey(1)
            if previous_board is not None:
                detect_move(previous_board, current_board, already_printed)  
...
def detect_move(previous_board, current_board, already_printed):
    differences = compare_boards(previous_board, current_board)
    for diff in differences:
        i, j, prev, curr = diff
        move_description = f"{i}{j}{prev}{curr}"
        if move_description not in already_printed:
            already_printed.add(move_description)
            if prev == '⬛' and curr in ['🟢', '🟣']:
                print(f"Jogador {'GREEN' if curr == '🟢' else 'PURPLE'} move da casa {chr(65 + j)}{8 - i} para nova posição.")
...
def main():
    ...
    while True:
        ...
        if not hand_presence and previous_hand_presence:
            # Processa a jogada somente se a mão foi retirada da visão da câmera
            frame_resized, previous_board, move_processed = process_frame(frame, detector, transformer, object_line_detector, previous_board, board_accumulator, already_printed, hand_presence)

        previous_hand_presence = hand_presence
        
         #SAÍDA DE TABULEIRO COM DETECÇÕES
        board_frame(frame, detector, transformer, object_line_detector, previous_board, already_printed)
